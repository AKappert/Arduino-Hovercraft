...
// Enums
enum MACHINE_STATE {STARTING, SCANNING, SHUTDOWN, CALCULATING, MOVING, PAUSING};
...
//   accAngleX = (atan(AccY / sqrt(pow(AccX, 2) + pow(AccZ, 2))) * 180 / PI) - 4.36; // AccErrorX ~(0.58) See the calculate_IMU_error()custom function for more details
//   accAngleY = (atan(-1 * AccX / sqrt(pow(AccY, 2) + pow(AccZ, 2))) * 180 / PI) + 19.00; // AccErrorY ~(-1.58)
//   // === Read gyroscope data === //
...
//   GyroX = GyroX + 5.28; // GyroErrorX ~(-0.56)
//   GyroY = GyroY - 0.64; // GyroErrorY ~(2)
//   GyroZ = GyroZ + 1.08; // GyroErrorZ ~ (-0.8)
